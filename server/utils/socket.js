  ...
  io.on('connection', async function (socket) {
    ...
    socket.on('joinChat', async () => {
      try {
        ...
        // 更新聊天状态
        await updateOnlineStatusById(id, true)

        let contents = await getContentBySendId(id)
        let conversition = []
        if (contents.state) {
    ...
    socket.on(
      'sendMsg',
      async ({ conversition, conversition: { recipientId } }) => {
        try {
          ...
          if (reciver.sessionHistory != null) {
            ...
            let len =
              historySessionList.filter((x) => x.id == userInfo.id)?.length ?? 0
            if (len === 0) {
              const datas = {
                ...userInfo,
                historySessionList: '',
    ...
    //修改信息阅读状态
    socket.on('changeMsgRead', async ({ id }) => {
      try {
        let contentResult = await getUnReadContent(id, userInfo.id)
      ...
      try {
        ...
        if (userResult.state && userResult.data && userResult.data.id) {
          ...
          if (userResult.data.sessionHistory != null) {
            historySessionList = JSON.parse(userResult.data.sessionHistory)
            let len =
              historySessionList.filter((x) => x.id == recipient.id)?.length ??
              0
            if (len == 0) {
              const handleData = {
                ...recipient,
